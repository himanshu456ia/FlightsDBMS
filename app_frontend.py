...
class Starting_window:
    def __init__(self, master=None):
        ...
        action_frame.pack(side='bottom', fill='both', padx=10, pady=5)

        # Labels
        self.lbl_title = tk.Label(title_frame, text="Flight Enquiry System", font='Arial 20 bold', bg='grey')
        self.lbl_title.grid(row=0, column=0, sticky='nwes', padx=20, pady=5)

        self.query_title = tk.Label(query_frame, text="What would you like to enquire about?", font='Arial 12')
        self.query_title.grid(row=0, columnspan=2, sticky='wnes', padx=40, pady=5)
        ...
        self.rdiobtn_flight.grid(row=1, column=0, sticky='nwes')
        self.rdiobtn_passengers = tk.Radiobutton(query_frame, text="Passengers", value='P', variable=self.v)
        self.rdiobtn_passengers.grid(row=1, column=1, sticky='nwes')        
